...
type PerNodeAllocatedClaims struct {
	sync.RWMutex
	allocations map[string]map[string]nascrd.AllocatedResources
}

func NewPerNodeAllocatedClaims() *PerNodeAllocatedClaims {
	return &PerNodeAllocatedClaims{
		allocations: make(map[string]map[string]nascrd.AllocatedResources),
	}
}
...
func (p *PerNodeAllocatedClaims) Get(claimUID, node string) nascrd.AllocatedResources {
	p.RLock()
	defer p.RUnlock()

	if !p.Exists(claimUID, node) {
		return nascrd.AllocatedResources{}
	}
	return p.allocations[claimUID][node]
}

func (p *PerNodeAllocatedClaims) VisitNode(node string, visitor func(claimUID string, allocation nascrd.AllocatedResources)) {
	p.RLock()
...
func (p *PerNodeAllocatedClaims) Visit(visitor func(claimUID, node string, allocation nascrd.AllocatedResources)) {
	p.RLock()
	for claimUID := range p.allocations {
...
func (p *PerNodeAllocatedClaims) Set(claimUID, node string, devices nascrd.AllocatedResources) {
	p.Lock()
	defer p.Unlock()

	_, exists := p.allocations[claimUID]
	if !exists {
		p.allocations[claimUID] = make(map[string]nascrd.AllocatedResources)
	}
